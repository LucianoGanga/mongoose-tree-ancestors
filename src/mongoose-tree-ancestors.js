...
function treeAncestors(schema, options) {
	...
	// 	let id = elOrId[options.field] || elOrId;
	// 	let self = this;
	// 	let promise = new mongoose.Promise;
	// 	if (callback)
	// 		promise.addBack(callback);
	// 	if (elOrId[parentId])
	// 		promise.complete(el[ancestors].length === 0);
	// 	else {
	// 		self.constructor.findOne(byId(id), addId({
	// 			path: 1
	// 		})).exec(function(err, doc) {
	// 			if (err || !doc)
	// 				return promise.error(err);
	// 			promise.complete(doc[ancestors].length === 0);
	// 		});
	// 	}
	// 	return promise;
	// });
	...
	// 	let promise = new mongoose.Promise;
	// 	let query = new Query(arguments);
	// 	if (query.callback)
	// 		promise.addBack(query.callback);
	// 	let self = this;
	// 	if (self.path.length > 2) {
	// 		let ancArray = self.path.substr(1).split(options.separator);
	...
	// 		let cursor = self.constructor.find(query.condition, query.fields)
	// 			.sort(query.sort);
